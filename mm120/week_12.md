# WEEK 12

    Author: Idris Ispandi
    April 20 2026

### Final Demo

As we were stting up for the Final Demo A short occured on an exposed wire near the battery terminal which happen to fry the ESP32 and LDO chip. This caused us to have to ask for a 1-day extension for the Final Demo.

Unfortunately we could not get the Board to work again in the short period of time s o we ended up having to use the Bench power supply.


Besides the fact that we had to use the bench supply, All the other requirements were fulfilled.

![alt text](images/final_demo.JPG.jpeg)

### Final Presentation
![alt text](images/final_pres.jpeg)

On this day we presented our findings and how our project worked and the general flow of how we developed our project. We answered any questions that were asked to us in regards to how we solve the issues in the project. It was a great run!

